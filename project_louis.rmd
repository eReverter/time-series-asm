...
```{r}
flu.usa.ts = ts(flu.usa$INF_ALL, start=1998, end=2020,frequency=52) #Lets not include covid for first analysis
serie = flu.usa.ts
```
...
```{r}
(mod1 <- arima(lnserie, order=c(5,1,0),
              seasonal=list(order=c(5,1,0),period = 52), method="CSS"))
```
