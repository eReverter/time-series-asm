...
MODEL 2: ARMA(3,1,3)(1,1,1)_52 (to be further optimized in Estimation Part)
...
Optimal model for now: p=3 d=1 q=3 P=1 D=1 Q=1. Let's look at it closer and see if all parameters are significantly contributing. 

```{r}
model2<-arima(lnserie, order = c(3,1,3), seasonal = list(order=c(1,1,1), period=52))
model2$aic
model2$coef
```

```{r}
cat("\nT-ratios:",round(model2$coef/sqrt(diag(model$var.coef)),2))
cat("\nSignificant?:",abs(model2$coef/sqrt(diag(model$var.coef)))>2)
```

```{r}
model2_coef_change<-arima(lnserie, order = c(3,1,3), seasonal = list(order=c(1,1,1), period=52), fixed=c(NA,NA,NA,NA,NA,NA,0,NA)) #Setting ar3 and sar1 to 0.
...
```
Similar AIC but simpler model. Continue.

```{r}
model2_coef_change<-arima(lnserie, order = c(3,1,3), seasonal = list(order=c(1,1,1), period=52), fixed=c(NA,NA,NA,NA,NA,0,0,NA)) #Setting ar3 and sar1 to 0.
model2_coef_change$aic
model2_coef_change$coef
cat("\nT-ratios:",round(model2_coef_change$coef/sqrt(diag(model2_coef_change$var.coef)),2))
cat("\nSignificant?:",abs(model2_coef_change$coef/sqrt(diag(model2_coef_change$var.coef)))>2)
...
model2 = model2_coef_change
```
Improved AIC and simpler model.
...
validation=function(model){
  ...
  ##Ljung-Box test
  cat("\nLjung-Box test\n")
  print(t(apply(matrix(c(1:6,(1:6)*s)),1,function(el) {
...
#Intervals
# We must apply the reverse transformation that we have done by taking exp.
# lower confidence band
tl<-ts((pr-1.96*se),start=ultim,freq=52) 
tu<-ts((pr+1.96*se),start=ultim,freq=52) 
...
# the black line is the true time series
# the red line is the prediction
# the blue lines are the confidence bands for the prediction

ts.plot(lnserie,tl,tu, pr,lty=c(1,2,2,1),col=c(1,4,4,2),type="o", xlim=c(2019, 2020)) #main=paste("Model #ARIMA(",paste(pdq,collapse=","),")(",paste(PDQ,collapse=","),")12",sep=""))
...
ts.plot(serie,tl,tu, pr,lty=c(1,2,2,1),col=c(1,4,4,2),type="o", xlim=c(2019, 2020)) #main=paste("Model #ARIMA(",paste(pdq,collapse=","),")(",paste(PDQ,collapse=","),")12",sep=""))
#abline(v=(ultim[1]-2):(ultim[1]+2),lty=3,col=4)
```
...
#ts(data.frame(LowLim=tl[-1],Predic=pr,UpperLim=tu[-1],Observ=obs,Error=obs-pr,PercentError=(obs-pr)/obs),start=ultim,freq=52)
...
modA=arima(lnserie, order = c(3,1,3), seasonal = list(order=c(1,1,1), period=52), fixed=c(NA,NA,NA,NA,NA,0,0,NA))
modB=arima(lnserie2, order = c(3,1,3), seasonal = list(order=c(1,1,1), period=52), fixed=c(NA,NA,NA,NA,NA,0,0,NA))
...
tu<-ts(exp(pr+1.96*se)-1,start=ultim,freq=52)
pr<-ts(exp(pr)-1,start=ultim,freq=52)
ts.plot(serie,tl,tu, pr,lty=c(1,2,2,1),col=c(1,4,4,2),type="o", xlim=c(2019, 2020))
