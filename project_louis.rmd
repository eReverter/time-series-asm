...
Optimal model for now: p=1 d=1 q=1 P=1 D=1 Q=1. Let's look at it closer and see if all parameters are significantly contributing.


Check that the intercept is not significant:
```{r}
model2<-arima(d1d52lnserie, order = c(1,0,1), seasonal = list(order=c(1,0,1), period=52))
model2$aic
model2$coef
```
...
```{r}
model2<-arima(lnserie, order = c(1,1,1), seasonal = list(order=c(1,1,1), period=52))
model2$aic
model2$coef
...
Can we remove sar1?
```{r}
model2_coef_change<-arima(lnserie, order = c(1,1,1), seasonal = list(order=c(0,1,1), period=52)) #Setting sar1 to 0.
...
```{r}
validation(model, d1d1d52lnserie)
```
...
```{r}
validation(model2, d1d1d52lnserie)
```
...
The long term predictions look as depicted below. Sadly, it is known that covid struck during this long term forecast, which looks quite different in reality, as shown by the red line. Less influenca cases were recorded since most cases were directly diagnosed as being covid to not saturate hospitals.
```{r}
par(mfrow=c(1,1))
